...
<body class="bg-background-light dark:bg-gray-900 font-display text-text-dark min-h-screen flex flex-col">
    ...
    <div class="relative flex min-h-screen w-full flex-col overflow-x-hidden max-w-md mx-auto sm:max-w-2xl p-4">
        ...
        <header class="flex items-center justify-between py-4">
            <button onclick="window.location.href='{{ url('/track') }}'"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
                <div class="flex flex-col items-center text-center">
                    ...
                    <div class="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-background-light dark:bg-gray-700 text-sm font-medium text-gray-600 dark:text-gray-300 mb-4 mt-1">
                        ...
                        {{ $bus->route ?? 'No Route Assigned' }}
                    </div>

                    <div class="flex items-center gap-2">
                        <span class="relative flex h-3 w-3">
                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full {{ $bus->status === 'active' ? 'bg-green-400' : 'bg-red-400' }} opacity-75"></span>
                            <span class="relative inline-flex rounded-full h-3 w-3 {{ $bus->status === 'active' ? 'bg-green-500' : 'bg-red-500' }}"></span>
                        </span>
                        <span class="text-sm font-bold uppercase tracking-wider {{ $bus->status === 'active' ? 'text-green-600' : 'text-red-600' }}">
                            {{ $bus->status ?? 'Offline' }}
                        </span>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col justify-center items-center gap-2">
                    ...
                    <div class="text-center">
                        ...
                        <p class="font-bold text-text-dark dark:text-white mt-1">{{ $bus->model ?? 'Unknown' }}</p>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col justify-center items-center gap-2">
                    ...
                    <div class="text-center w-full">
                        ...
                        <div class="flex items-end justify-center gap-1">
                            <span class="font-bold text-xl text-primary">{{ $bus->current_passengers ?? 0 }}</span>
                            <span class="text-gray-400 text-sm mb-1">/ {{ $bus->capacity ?? 0 }}</span>
                        </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-sm text-gray-500 truncate">
                            {{ $bus->driver_contact ?? 'No Contact Info' }}
                        </p>
                    </div>

                    @if(!empty($bus->driver_contact))
                    <a href="tel:{{ $bus->driver_contact }}" class="size-10 rounded-full bg-primary text-black flex items-center justify-center hover:bg-opacity-80 transition-colors">
